...
<div class="space-y-8">
    <div>
        ...
        <h1 class="text-2xl font-serif tracking-wide text-slate-900">Create Flash Sale</h1>
    </div>

    <form method="POST" action="{{ route('admin.flash-sales.store') }}" enctype="multipart/form-data" x-data="flashSaleForm()">
        @csrf
                ...
                <div class="bg-white border border-slate-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] focus:outline-none focus:border-slate-900">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold tracking-[0.15em] uppercase text-slate-500 mb-2">Description</label>
                            <textarea name="description" rows="3" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 text-[13px] focus:outline-none focus:border-slate-900">{{ old('description') }}</textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[10px] font-bold tracking-[0.15em] uppercase text-slate-500 mb-2">Starts At</label>
                                <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" required class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] focus:outline-none focus:border-slate-900">
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold tracking-[0.15em] uppercase text-slate-500 mb-2">Ends At</label>
                                <input type="datetime-local" name="ends_at" value="{{ old('ends_at') }}" required class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] focus:outline-none focus:border-slate-900">
                            </div>
                        </div>
                        <div>
                    ...
                    <template x-for="(product, index) in products" :key="index">
                        <div class="flex items-start gap-4 p-4 bg-slate-50 mb-3">
                            <div class="flex-1 grid grid-cols-4 gap-3">
                                ...
                                <div>
                                    <input type="number" :name="'products['+index+'][sale_price]'" x-model="product.sale_price" placeholder="Sale Price" required step="0.01" class="w-full h-10 px-3 bg-white border border-slate-200 text-[12px]">
                                </div>
                                <div>
                                    <input type="number" :name="'products['+index+'][quantity_limit]'" x-model="product.quantity_limit" placeholder="Qty Limit" class="w-full h-10 px-3 bg-white border border-slate-200 text-[12px]">
                                </div>
                            </div>
                            <input type="hidden" :name="'products['+index+'][per_user_limit]'" value="2">
                            <button type="button" @click="removeProduct(index)" class="p-2 text-red-500 hover:text-red-700">
...
function flashSaleForm() {
    return {
        products: [{ product_id: '', sale_price: '', quantity_limit: '', per_user_limit: 2 }],
