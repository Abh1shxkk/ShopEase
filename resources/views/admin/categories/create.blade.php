...
<div class="max-w-2xl">
    <h1 class="text-2xl font-serif tracking-wide text-slate-900 mb-8">Create Category</h1>

    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="bg-slate-50 p-8">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 bg-white border border-slate-200 text-[13px] focus:outline-none focus:ring-1 focus:ring-slate-900 resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400 mt-2">Recommended: 400x400px, JPG or PNG</p>
            </div>

            <div class="grid grid-cols-2 gap-6">
                <div>
                    <label class="block text-[11px] font-bold tracking-widest uppercase text-slate-500 mb-3">Sort Order</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="w-full h-12 px-4 bg-white border border-slate-200 text-[13px] focus:outline-none focus:ring-1 focus:ring-slate-900">
                </div>
                <div>
