...
<div x-data="{ imagePreview: null }">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-2xl font-serif tracking-wide text-slate-900">Add New Product</h1>
            <p class="text-[12px] text-slate-500 mt-1">Create a new product listing</p>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-200 p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-900 transition-colors @error('name') border-red-300 @enderror" placeholder="Enter product name" required>
                        ...
                        <div>
                            ...
                            <input type="number" id="stock" name="stock" value="{{ old('stock', 0) }}" min="0" class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] text-slate-900 focus:outline-none focus:border-slate-900 transition-colors @error('stock') border-red-300 @enderror" required>
                ...
                <div class="bg-white border border-slate-200 p-6">
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="number" id="price" name="price" value="{{ old('price') }}" min="0" step="0.01" class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-900 transition-colors @error('price') border-red-300 @enderror" placeholder="0.00" required>
                        ...
                        <div>
                            ...
                            <input type="number" id="discount_price" name="discount_price" value="{{ old('discount_price') }}" min="0" step="0.01" class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-900 transition-colors @error('discount_price') border-red-300 @enderror" placeholder="0.00">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white border border-slate-200 p-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="status" value="active" {{ old('status', 'active') == 'active' ? 'checked' : '' }} class="w-4 h-4 text-slate-900 border-slate-300 focus:ring-slate-900">
                            <span class="text-[13px] text-slate-600 group-hover:text-slate-900 transition-colors">Active</span>
                        </label>
                        <label class="flex items-center gap-3 cursor-pointer group">
                            <input type="radio" name="status" value="inactive" {{ old('status') == 'inactive' ? 'checked' : '' }} class="w-4 h-4 text-slate-900 border-slate-300 focus:ring-slate-900">
                        ...
                        <div class="aspect-square bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center">
                            ...
                            <template x-if="!imagePreview">
                                <div class="text-center p-6">
                                    ...
                                    <p class="text-[12px] text-slate-400">No image selected</p>
                                </div>
                            </template>
                        </div>
                        <div>
                            <label class="text-[10px] font-bold tracking-[0.15em] uppercase text-slate-400 block mb-2">Image URL</label>
                            <input type="text" name="image_url" value="{{ old('image_url') }}" placeholder="https://example.com/image.jpg" class="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-[13px] text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-900 transition-colors" @input="imagePreview = $event.target.value">
                        </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button type="submit" class="flex-1 h-11 px-6 bg-slate-900 text-white text-[11px] font-bold tracking-[0.15em] uppercase hover:bg-slate-800 transition-colors">Add Product</button>
                </div>
            </div>
        </div>
    </form>
