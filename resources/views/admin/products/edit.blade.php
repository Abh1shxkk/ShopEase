...
@section('content')
<div x-data="{ imagePreview: '{{ $product->image ? Storage::url($product->image) : '' }}' }">
    <!-- Header -->
        ...
        <div class="grid lg:grid-cols-3 gap-6">
                ...
                <div class="card p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" class="input w-full @error('name') input-error @enderror" placeholder="Enter product name" required>
                            @error('name')<p class="text-sm text-red-500 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label for="description" class="label mb-2 block">Description <span class="text-red-500">*</span></label>
                            <textarea id="description" name="description" rows="4" class="input w-full min-h-[120px] @error('description') input-error @enderror" placeholder="Enter product description" required>{{ old('description', $product->description) }}</textarea>
                        ...
                        <div>
                            ...
                            <input type="number" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" min="0" class="input w-full @error('stock') input-error @enderror" required>
                ...
                <div class="card p-6">
                    ...
                    <div class="grid sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="price" name="price" value="{{ old('price', $product->price) }}" min="0" step="0.01" class="input w-full @error('price') input-error @enderror" placeholder="0.00" required>
                        ...
                        <div>
                            ...
                            <input type="number" id="discount_price" name="discount_price" value="{{ old('discount_price', $product->discount_price) }}" min="0" step="0.01" class="input w-full @error('discount_price') input-error @enderror" placeholder="0.00">
            ...
            <div class="space-y-6">
                <div class="card p-6">
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center space-x-3 cursor-pointer">
                            <input type="radio" name="status" value="active" {{ old('status', $product->status) == 'active' ? 'checked' : '' }} class="w-4 h-4 text-blue-600">
                            <span class="text-gray-700">Active</span>
                        </label>
                        <label class="flex items-center space-x-3 cursor-pointer">
                            <input type="radio" name="status" value="inactive" {{ old('status', $product->status) == 'inactive' ? 'checked' : '' }} class="w-4 h-4 text-blue-600">
                ...
                <div class="flex space-x-3">
                    ...
                    <button type="submit" class="btn btn-primary flex-1">Update Product</button>
                </div>
            </div>
        </div>
    </form>
