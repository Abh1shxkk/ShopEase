...
<div x-data="{ showDeleteModal: false, deleteId: null, deleteName: '' }">
    ...
    <div class="card p-4 mb-6">
        <form method="GET" class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search products..." class="input w-full">
            </div>
            <select name="category" class="input select w-full sm:w-48">
                <option value="">All Categories</option>
                @foreach($categories as $cat)
                <option value="{{ $cat }}" {{ request('category') == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                @endforeach
            </select>
            <select name="status" class="input select w-full sm:w-40">
                    ...
                    <tr>
                        ...
                        <td>
                            <div>
                                ...
                                <span class="text-sm text-green-600 block">${{ number_format($product->discount_price, 2) }}</span>
                                @endif
                            </div>
                        </td>
                        <td>
                            <span class="{{ $product->stock < 10 ? 'text-red-600 font-medium' : 'text-gray-600' }}">{{ $product->stock }}</span>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="flex items-center justify-end space-x-2">
                                <a href="{{ route('admin.products.edit', $product) }}" class="btn btn-ghost btn-icon" title="Edit">
                                ...
                                <button @click="showDeleteModal = true; deleteId = {{ $product->id }}; deleteName = '{{ $product->name }}'" class="btn btn-ghost btn-icon" title="Delete">
            ...
            <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6">
                <div class="text-center">
                    ...
                    <div class="flex space-x-3">
                        ...
                        <form :action="'{{ route('admin.products.index') }}/' + deleteId" method="POST" class="flex-1">
                            @csrf
                            @method('DELETE')
